...
export class Flatly {
     ...
     * @private
     */
    static _addTableMeta(table, tableName) {
     ...
     * @private
     */
    static _addRowMeta(row, tableName) {

        let meta = {
            table: tableName
        };

        if(_.isUndefined(row.flatly__)) {
            row.flatly__ = meta;
        }
    ...
    /**
     * Database initialisation method. Given an options object with {name} and {src} string params
     * internally it loads all the JSON files in the filepath specified by {src} by appending `/**\/*.json`
     * to {src} and using glob.
     *
     * @function Flatly#use
     * @param name {string} Specify a name for the database
     * @param src {string} A path to the required data
     * @returns {Flatly}
     */
    use(name, src) {
        ...
        this._baseDir = path.resolve(global.parent, src);

        this._files = io.getAll({src: this._baseDir});

        let _tables = io.parse(name, this._files);
     ...
     * If {matchBy} is {undefined} Flatly will attempt to use the 'id' propery of the {rowData} argument
     * and finally throw an Exception on failure.
